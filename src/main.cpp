...
bool CheckTransaction(const CTransaction& tx, CValidationState &state)
{
    ...
    if (tx.IsCoinBase())
    {
        if (tx.vin[0].scriptSig.size() < 2 || tx.vin[0].scriptSig.size() > 215)          //fix as per orig newyorkcoin main.cpp line 584
...
void static UpdateTip(CBlockIndex *pindexNew) {
    ...
    if(chainActive.Height() % 15 == 0 || chainActive.Height() > 3000000){
    LogPrintf(" Height=%d  log2_work=%.8g  tx=%lu  date=%s progress=%f  cache=%u\n",
      chainActive.Height(), log(chainActive.Tip()->nChainWork.getdouble())/log(2.0), (unsigned long)chainActive.Tip()->nChainTx,
      DateTimeStrFormat("%Y-%m-%d %H:%M:%S", chainActive.Tip()->GetBlockTime()),
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    // Newyorkcoin: (mainnet >= 710000, testnet >= 400000, regtest uses supermajority)
